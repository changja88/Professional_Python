...
# - 집합도 해시 테이블을 이용해서 구현하므로, 이장에서는 집합도 다룬다.
# - 해시 테이블이 작동하는 방식을 알아야 딕셔너리와 집합을 최대로 활용할 수 있다
# - 메모리 공간 효율성은 높지 않다 -> 키 검색이 아주 빠르다(속도를 위해 공간을 포기하는 예)
#       - 메모리에 올리기는 힘들지만 일단 올라오면 딕트 크기와 무관하게 빠른 접근 속도를 제공한다
# - 딕트에 항목을 추가하면 기존 키의 순서가 변경될 수 있다
#       - 항목이 추가될 때 마다 파이썬 인터프리터는 그 딕트의 해시 테이블 크기를 늘릴지 판단한다
#         더큰 해시 테이블을 새로 만들어서 기존 항목을 모두 새 테이블에 추가한다
#         이 과정 동안 기존과 다르게 해시 충돌이 발생해서 새로운 해시 테이블에서의 키 순서가 달라 질 수 있다
